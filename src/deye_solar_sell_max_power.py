...
class DeyeSolarSellMaxPowerEventProcessor(DeyeEventProcessor):
    """
    Handles "solar sell max power" parameter modification requests received through MQTT
    """
    ...
    def __init__(
        ...
        self.__solar_sell_max_power_topic_suffix = "settings/solar_sell_max_power"
        self.__sensors = sensors
        self.__solar_sell_max_power_sensor_reg_addr = None

    def get_id(self):
        return "solar_sell_max_power"

    def get_description(self):
        return "Solar sell max power setting over MQTT"
    ...
    def initialize(self):
        ...
        if len(matching_sensors) == 0:
            ...
            return
        self.__solar_sell_max_power_sensor_reg_addr = matching_sensors[0].get_registers()[0]
        self.__mqtt_client.subscribe_command_handler(
            self.__logger_config.index, self.__solar_sell_max_power_topic_suffix, self.handle_command
        )

    def handle_command(self, client: Client, userdata, msg: MQTTMessage):
        if self.__solar_sell_max_power_sensor_reg_addr is None:
            return
        try:
        ...
        if solar_sell_max_power < 0 or solar_sell_max_power > 12000:
            self.__log.error(f"Invalid value for solar sell max power setting: {solar_sell_max_power}")
            return
        self.__log.debug(f"Setting solar sell max power to {solar_sell_max_power}")
        success = self.__modbus.write_register_uint(self.__solar_sell_max_power_sensor_reg_addr, solar_sell_max_power)
        if success:
            self.__log.info(f"Solar sell max power updated to {solar_sell_max_power}")
        else:
            self.__log.error(f"Failed setting solar sell max power to value: {solar_sell_max_power}")
